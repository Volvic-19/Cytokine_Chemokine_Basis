...
message("show warnings after shrinkage:")
warnings()

message("start removing gwas.DT")
rm(gwas.DT)
...
message("start writing shrinkage.DT!")
fwrite(shrinkage.DT, "../manifest/shrinkage.DT.tsv.gz", sep="\t")
message("finished writing!")


message("show warnings after writing shrinkage.DT:")
tail(warnings(), 2)

# metrix = shrinkage * beta
# filter shrinkage.DT
message("start filtering shrinkage.DT!")
shrinkage.e5 <- shrinkage.DT[ shrinkage > 1e-5]

shrinkage.e6 <- shrinkage.DT[ shrinkage > 1e-6]

message("finished filtering!")
message("show warnings after filtering:")
tail(warnings(), 2)

# remove shrinkage.DT
message("removing shrinkage.DT")
rm(shrinkage.DT)

# Save shrinkage metrics

message("start writing shrinkage.e5.DT!")
fwrite(shrinkage.e5, "../manifest/shrinkage.e5.DT.tsv.gz", sep="\t")
message("finished writing!")

message("start writing shrinkage.e6.DT!")
fwrite(shrinkage.e6, "../manifest/shrinkage.e6.DT.tsv.gz", sep="\t")
message("finished writing!")

message("show warnings after writing:")
tail(warnings(), 2)
